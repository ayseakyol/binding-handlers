try {
  ...
  const obj = {
    ...
    }, // there are 256 characters in charCode and we can understand from tests
    renderCharCode: function() {
      // 1 mistake
      return `<text>${String.fromCharCode(this.state.charCode)}</text>`; // use charCode
    },
    handler: function(display, event) {
      // 2 mistakes
      // debugger;
      this.state.num = Number(event.target.value);
      this.numToCharCode(); // use numToCharCode function
      display.innerHTML = this.renderCharCode(); // in UI renderCharCode result should be seen
    ...
    view: function(id) {
      ...
      inputEl.type = "number";
      inputEl.value = "0";
      inputEl.onchange = this.handler.bind(this, outputEl); // while using call we must use [];
