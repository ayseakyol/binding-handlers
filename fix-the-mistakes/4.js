try {
  ...
  const obj = {
    ...
    add: function() {
      // 1 mistake
      this.state.num += this.state.step; // add with =
    },
    subtract: function() {
      // 1 mistake
      this.state.num -= this.state.step; // subtract with =
    },
    handleClick: function(display, event) {
      // 1 mistake
      // debugger;
      const action = event.target.innerHTML; // take from HTML
      this[action]();
    ...
    handleStepChange: function(event) {
      // 1 mistake
      this.state.step = Number(event.target.value); // change value to number
      this.log.push({
        action: "set step",
        newState: JSON.parse(JSON.stringify(this.state))
      });
    ...
    view: function(id) {
      ...
      stepSizeEl.onchange = this.handleStepChange.bind(this, displayEl); // bind
