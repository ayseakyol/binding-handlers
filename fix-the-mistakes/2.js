try {
  ...
  const obj = {
    ...
    updateState: function(num, mod) {
      // 5 mistakes
      this.state.number = num; // this.number = undefined
      this.state.modulo = mod; // they are in state
      this.state.result = num % mod; // this.num = undefined. they must be num and mod
    },
    handler: function(event) {
      // 2 mistakes
      // debugger;
      const form = event.target.form; // target must be added
      const newNum = Number(form.newNum.value);
      const newMod = Number(form.newMod.value);
      this.updateState(newNum, newMod);
      form.lastChild.innerHTML = this.state.result; // in innerHTML
    ...
    view: function(id) {
      ...
      outputEl.style.marginLeft = "5%"; // adding css we must write style
      ...
      container.appendChild(formEl); // we must appen formEl
